...
class UserControllerTest {
    ...
    private fun addUser(name: String, email: String): HttpResponse<JsonNode> {
        return Unirest.post(origin + "/api/users").body("{\"name\":\"$name\", \"email\":\"$email\"}").asJson()
    }
    private fun updateUser(id: Int, name: String, email: String): HttpResponse<JsonNode> {
        return Unirest.patch(origin + "/api/users/$id").body("{\"name\":\"$name\",\"email\":\"$email\"}").asJson()
    }

    // helper function to delete a testuser from the database
    ...
    inner class ReadUsers {
        ...
        fun `get all users from the database returns 200 or 404 response`() {
            ...
            if (response.status == 200) {
                val retrieveUsers: ArrayList<User> = jsonToObject(response.body.toString())
                assertNotEquals(0, retrieveUsers.size)
            } else {
        ...
        @Test
        fun `get user by id when user exists returns 200 response`() {
            ...
            // After -restore the db to the previous state by deleting the added user
        ...
        @Test
        fun `get user by email when email exists returns 200 response`() {
            // Arrange - add the user
    ...
    inner class UpdateUsers {
        @Test
        fun `Updating a user when it exists, returns 204 response`() {
            ...
            // Act & Assert - update name and email of the retrieved user and assert 204 is returned
            assertEquals(204, updateUser(addedUser.id, updatedName, updatedEmail).status)

            // Act & Assert - retrieve updated user and assert the details are correct
        ...
        @Test
        fun `Updating a user when it does not exist, returns 404 response`() {
            // Act & Assert - attempt to update name and email of the user that doesnt exist
